...
using NotificationService.gRPC.Validators;
...
try
{
    ...
    app.MapGet("/", () => "NotificationService gRPC - Puerto 7005. Usa un cliente gRPC para comunicarte.");

    // Health check endpoint
    app.MapGet("/health", async (NotificationDbContext dbContext) =>
    {
        try
        {
            ...
            {
                status = "Healthy",
                service = "NotificationService.gRPC",
    ...
    app.Logger.LogInformation("║        NotificationService.gRPC INICIADO                  ║");
    app.Logger.LogInformation("╚═══════════════════════════════════════════════════════════╝");
    app.Logger.LogInformation("🚀 Servicio escuchando en puerto 7005");
    app.Logger.LogInformation("📊 Base de datos: ECommerceNotifications");
    app.Logger.LogInformation("📧 Mock habilitado: Email y SMS simulados (no envío real)");
    app.Logger.LogInformation("");
